...
def plot_fit_result(
    ...
    fit_model_render_im = np.sum(list(fit_model_render.values()), axis=0)
    ...
    lm = s([
        min(
            ...
            fit_model_render_im.min()
        ),
        max(
            np.nanmax(galaxy_data),
            fit_model_render_im.max()
        )
    ])
    ax[0, 0].imshow(
        s(galaxy_data),
    ...
    ax[0, 1].imshow(
        s(fit_model_render_im),
        vmin=lm[0], vmax=lm[1], cmap='gray_r', origin='lower'
    )
    d = (galaxy_data - np.array(fit_model_render_im)) / sigma_image
...
def do_subject(subject_id):
    ...
    fit_result1 = o.do_fit(keys=keys1, p0=p1, options=dict(maxiter=2000))
    final_params = pd.DataFrame(fit_result1['fit_model']).unstack().dropna()

    plot_fit_result(
        subject_id, fm.galaxy_data, fm.sigma_image, fm.psf,
        o, agg_res,
        agg_res.model, fit_result1['fit_model_uncorrected']
    )
...
if __name__ == '__main__':
    ...
    main(args.subjects or fitting_metadata.index, check=~bool(args.subjects))
